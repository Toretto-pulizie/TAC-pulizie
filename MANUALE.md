...
- **Filtro e ordinamento come in Excel**: cliccando la freccina nell'intestazione di una colonna si apre un menu con "Ordina crescente/decrescente" e un elenco di caselle (una per ogni valore presente in quella colonna) per mostrare solo le righe con i valori scelti — con una casella di ricerca per trovare velocemente un valore nell'elenco. Un pallino accanto al nome della colonna indica che un filtro è attivo su quella colonna. Funziona così su tutte le tabelle di Anagrafiche, Preventivi e Consuntivi.
