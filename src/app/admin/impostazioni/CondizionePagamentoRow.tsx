...
import { updateCondizionePagamento, deleteCondizionePagamento } from "@/app/actions/condizioniPagamento";

export function CondizionePagamentoRow({
  id,
  etichetta,
}: {
  id: string;
  ...
  const [state, action, pending] = useActionState(updateCondizionePagamento, undefined);
        ...
        <button
          ...
          onClick={() => {
            if (confirm("Eliminare questa voce?")) {
              startDeleteTransition(() => deleteCondizionePagamento(id));
            }
          }}
